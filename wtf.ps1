...
function Write-WtfWorkspace {
    ...
    $ws = @{
        ...
        settings = @{
            ...
            # SCM hygiene: don't let VS Code wander up from a worktree and open
            # its source repo, and skip submodule sub-repos. (We deliberately do
            # NOT set git.autoRepositoryDetection=false — that would also hide the
            # real worktree repos from Source Control.)
            'git.openRepositoryInParentFolders' = 'never'
            'git.detectSubmodules'              = $false
            'files.exclude'                     = @{ '**/.wtf-meta.json' = $true }
        }
    }
...
> Auto-generated by ``wtf``. Use this file as the source of truth for the
> feature so any fresh agent session can resume with full context.

**Project:** $Project
**Apps in scope:** $($Apps -join ', ')
...
_What problem does this feature solve? What does "done" look like?_

## 🧠 Decisions

_Architectural choices made during planning. Lock these in before coding so
agents don't re-litigate them every session._
...
_Things to resolve. Leave a breadcrumb when you defer._

## 📝 Files Touched

_Update as you go. Helps new agent sessions understand scope._

## 🧪 Testing Notes

_Manual steps to verify. Edge cases. Things to check before PR._

---

## Agent Session Log

_Optional — paste short notes when you compact or restart an agent session._
"@
...
function Invoke-WtfCreate {
    ...
    Write-WtfWorkspace -WorkspacePath $workspaceFp -FeatureDir $featureDir -Worktrees @($wtList) -Deps @($depNorm)
...
function Invoke-WtfAdd {
    ...
    Write-WtfWorkspace -WorkspacePath $wsPath -FeatureDir $featureDir -Worktrees @($layout.Worktrees) -Deps @($layout.Deps)
...
function wtf {
    ...
    param(
        ...
        [Parameter(Position=1)][string]$Context,
        [Parameter(Position=2)][string]$Project,
        [Parameter(Position=3)][string]$Branch,
    ...
    switch ($Action.ToLower()) {
        ...
        'list'   { Invoke-WtfList }
        'ls'     { Invoke-WtfList }
        'doctor' { Invoke-WtfDoctor -Fix:($Rest -contains '-Fix' -or $Rest -contains '--fix') }
